...
def opening_guide() -> str:
    ...
    追加；过期后仍可用 `rules(开局指南)` 取回（走 `_help_sections` 的章节表）。
...
def _help_sections() -> list[tuple[str, str]]:
    ...
    sections = [
        ("总览", (
            ...
            "空行隔开），每行一格、**自带语义、零解码**——"
            "`(x,y)归属地形，[L2城][，地名][，番号…]`（归属=我 / 野(无主) / 国名；番号垫在最后；"
            "番号=短兵种+军队番号，如 `步1、骑5、楚骑3`，可对上【军队】面板的 `#n`；"
            "格主的部队省前缀并排前面；无主地写作「野人」；**只列视野内的格**）。"
            ...
            "军队/威胁面板、战报的格子标记、冲入交战的回执、mv 撞墙的报错、占领回执；"
            "（打一格看不见的敌方地，也会在回执/战报里看到那格的城——打了才知道；"
        ...
        ("军队与战斗", (
            ...
            f"非交战且在**本国补给没断供**时每回合回血 +{ARMY_HEAL_PER_TURN}HP，"
            f"**回血自动扣装备**：每支真吃到回血的军队每回合 {HEAL_EQUIP_COST} 件"
            f"（满血的不扣、断供或交战中的不回血也不扣、**装备仓见底就停止回血**；"
            f"**民兵除外**——它征召就不吃装备，回血也不吃）；"
            f"断粮则每军按缺口比例扣血"
        ...
        # 开局指南：前 20 回合由 system_prompt 强行挂载（World.opening_guide），
        # 过期后**还能用 rules(开局指南) 取回**——所以它同时住在这里。
        ("开局指南", opening_guide()),
    ]
    return sections
