...
TOOL_SCHEMAS = [
    ...
    {"type": "function", "function": {
        "name": "build", "description": "在自己的一块地上建一座建筑。每地块每回合限建1座。建筑: 城堡/林场/农场/矿场/黄金矿场/石油厂/木材能源厂/石油能源厂/补给厂/装备厂/兵营/市政厅/瞭望塔/外交中心/工程院/军屯。采集类上限=本地资源量；补给厂/装备厂/能源厂任地可建（工业不挑地）；兵营需本地已用建筑位≥3；瞭望塔=事件视野+4圆；市政厅需本地已用位≥6且每地块限1；外交中心=外交费减半可叠加但自建全国限1（第2座只能抢）；工程院=本地建造费-20%需本地位≥6；军屯=可征民兵(50金/支)且其驻本格不耗补给（上限=本地耕地）。",
...
def _default_system_prompt(world, name) -> str:
    return (
        ...
        "撤出攻守对等：交战中的军队（含防守方）离开战场一律用 retreat（固定只能退相邻1格，回合末随战斗结算后脱离、防御方撤退减伤50%；撤退军本回合输出-80%）；mv 不能从交战地撤离；"
