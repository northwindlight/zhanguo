...
HUNS_DOCTRINE = (
    ...
    "· 核心是运动战：骑兵动 2 格、集中决战、打完就撤（retreat 固定只退相邻 1 格、耗移动，"
    "回合末随战斗结算后脱离；防御方撤退减伤 50%），不恋战。集中骑兵挑软柿子（敌方分散/兵力少/"
    "补给差），避免硬拼满员要塞（山地+城堡减伤可达 75%）。见缝插针多抢无驻军之地，少打有损失之战。\n"
    "· **议和只跟能谈的谈，且绝不亏本**：拒绝谈判/只肯白和/不回信的国家，**别再发第二次——"
    "打疼了再说**（占它产金地、灭它野战军，疼了它自己来求）。停战期对方一分不给，你的骑兵照吃补给"
