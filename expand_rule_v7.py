...
MIL_SHARE_MIN = 0.20  # 军费占收入的下限（低于它 → 军队太小，补征）
MIL_SHARE_MAX = 0.60  # 上限（高于它 → 停止扩军，专心复利）
...
def expand_rule_turn_v7(world, name: str, rng: random.Random | None = None,
    ...
    target, target_pb = best_build(
        world, [bn for bn in ("黄金矿场", "矿场", "林场", "农场", "石油厂")
                if cnt(bn) < 200])
    must_keep = {
        "粮食": sup_n * 2 + planned_rec * 10 + 10,
        "矿石": (sup_n + eqp_n) * 2 + 10,
        "石油": eqp_n * 2 + 4,
        # **目标的木料** + 能源厂燃料 —— 由选定的目标算出来，不是固定值
        "木头": (BUILDINGS[target]["wood"] if target else 0) + cnt("木材能源厂") * 2,
        "补给": army_n * 2 + 5,
        "装备": planned_rec * 5 + 5,
    }
    ...
    if R()["装备"] < 20 and R()["黄金"] >= 300:
        buy("装备", 20 - int(R()["装备"]))
    # **口粮优先于一切**（v7 改动）：v6 原文是 `and R()["黄金"] >= 200` ——
    # 钱不到 200 就不买口粮，实测于是反复饿死整支军队（seed3 覆没 3 次）。
    # 军费是每回合重复的长期支出，建造是一次性的：**先喂兵，再建设**。
    if R()["补给"] < army_n * 12:
        buy("补给", army_n * 12 - int(R()["补给"]), reserve=0)
