...
★ **另有两条与体积无关的根因，别只盯 replay 大小**（2026-09-19 实测补记）：
  ...
  (b) **归档压在 replay 之前，它一变后面全废**：`render_archive` 的内容随
      `before_turn`（= replay 起点）变；窗口被预算卡住时，replay 起点每回合都能
      往前挪 → 归档每回合改字节 → 整个 replay + 状态全 miss（实测那一回合命中 6.9%，
      相邻回合 94%）。所以要盯的不是"可命中前缀 ≈93%"，而是压缩周期内**有没有
      每回合都在挪起点**（`describe()` 里的 replay 回合数是否每回合 -1）。
