...
世界市场（无建筑门槛，随时可交易）：黄金是货币。可交易=全部可存储物资：
  ...
  产金路：黄金矿场 +10金/回合/座；市政厅（高密度城，需本地已用位>5）按已占建筑位每座 +1金/回合；卖物资也能换黄金（分批卖更划算）。
建筑（造价金·耗木·上限）：
  ...
  市政厅 500·40木 每地块限1座·需本地已用位≥6 → 每回合按已占建筑位×1金（不含市政厅；维持1能源）
    ...
    while True:
        ...
        try:
            if cmd in ALIAS["quit"]:
                ...
                if r.get("hall_gold"):
                    print(f"  市政厅收入 +{r['hall_gold']}（按本地已占建筑位×1金；电网不足则停摆），国库 {world.gold}")
                if r["famine"]:
