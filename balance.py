...
ACADEMY_OPT_DENSITY = 7
#   工程院「该建了」的密度（用户 2026-09-16 判据：**6~8 个建筑时最该建**，取中 7）。
#   为什么要它：`剩余位 × 本格均造价` 单独用是**单调递减**的（一格就建最划算）——
#   实测 seed 900000 t=250：该乘积在 n=1 处最大（1,260），而 n=6~8 只有 670~920。
#   真实节奏是：本格先被便宜的采集楼占满（折扣浪费），6~8 座之后才轮到工厂/兵营/市政厅
#   那些贵的 ⇒ 折扣才开始值钱。这个因子 = 「到 7 座才算进入该建区间」。
