...
class WindowTransformer(nn.Module):
    ...
    def __init__(self, win_widths: dict[str, int], *, d_model: int = 192, n_layer: int = 4,
        ...
        # ★candx 新增：候选互见层的归一化（唯一新参数：2×d_model=384）。
    ...
    def forward(self, win: dict, cand: dict, cand_mask: torch.Tensor | None = None,
        ...
        **默认 False**：现有所有调用点（`forward_batch`、`bc.py`、`train.py`）
        ...
        # ---- candx：候选互相可见 ----
        # Q/K/V 全用 h，**复用 self.cross**（权重绑定，零新注意力参数）；新增参数只有 ln_cand。
        # key_padding_mask 用 cand_mask 取反：padding 候选不许当 K/V 被看见（它们那行是垃圾）。
        # 代价 O(K²·d)：K(160~400) 与 T(≈360) 同量级，K<T 时这步并不比上一步贵。
        # 语义：动作选择的分数从此可以表达"这个比那个好，因为那个此刻点不动/更贵/已被引用"。
        cm = None if cand_mask is None else ~cand_mask.to(h.device)
        att2, _ = self.cross(h, h, h, key_padding_mask=cm, need_weights=False)
