...
### ★★2026-09-17 **结案：判据不走了**（用户：「先 indep 凑合，没有那么多机器」）

**落 indep（`3c89f7a`）是一个决定，不是测量结果** —— 上面那条判据**一步没走**，
`‖μ‖²` 的 z 从来没比过（没有 tied 对照炉）。**别把"落了 indep"读成"indep 赢了 tied"。**

当时讨论补记（免得重推）：tied 强制 `W_B := W_A` ⇒ **tied ⊂ indep 是严格包含**，
「绑权重压表达力」在参数化层面**可证**，但**只说"存在 indep 能表达而 tied 不能的函数"，
没说那个函数在这任务里重要**；另一面 tied 是**多任务共享表征**，干扰必然存在而**符号理论定不了**
（共享结构/正则 vs 两个任务拉扯）。⇒ "indep 更好"只能实测，实测没钱做 ⇒ 按决定留 indep。

**要翻案**：补一炉 tied 的从零 BC → 两炉各跑 `probe_grad_noise_v2` 12 局 → 按上面判据判读。
