# -*- coding: utf-8 -*-
"""v12 = **v10 + 拿掉 4 处 `int()` 截断**（把浮点价格截成整数）。

## 为什么另开一代，而不是改 v10

`ruleai/__init__.py` 的纪律第 2 条：**v10 是 BC 老师，一个字节都不许动**
（改它 = 作废全部历史标签）。所以修好的那份**开成新代 v12**，v10 原样冻结。

## 改了什么（4 处，全在 `int(world.prices[...])`）

    int(1.60) = 1   ⇒ 木价被看低 37%（S=1）      ← 病灶
    int(18.19) = 18 ⇒ 只低 1%（S=10，被放大后自然变小）
⇒ 老师说"排建筑预算"算错 ⇒ `need=0` ⇒ 不排楼、木全卖掉、不买回。

实测（8 图 200 回合，同 seed）：**v10 = 21,213 → v12 = 20,383（−3.9%）**。
⚠ 注意方向：**v12 比 v10 差 3.9%** —— 这不是"把老师修强了"，是**把老师修诚实了**
（v10 靠这个算术 artifact 多花了钱）。⇒ **拿 v12 当基准时，验收线随之下移。**

## 用法

`--teacher v12`（已注册进 `rule_ai.py` 的 `_SOURCES`）。**v10 仍然可用、且未被改动。**
"""
