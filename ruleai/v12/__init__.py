# ★ v12 = v11 + 拿掉 4 处 `int()` 截断（把浮点价格截成整数）
#   理由与 v10→v12 同（见 ruleai/v12/economy.py 的注释）：
#     int(1.60)=1 把木价看低 37%（S=1），而 int(18.19)=18 只低 1%（S=10）
#   ⇒ 老师说"排建筑预算"算错 ⇒ need=0 ⇒ 不排楼、木全卖掉、不买回。
#   ★另开一代而不是改 v11：与 `ruleai/__init__.py` 的纪律同（改某一代 = 作废它的历史标签）。
# -*- coding: utf-8 -*-
"""v11：**经济层与军事层分离**的规则 AI（设计底稿 `docs/v11编组模型.md`）。

    v11/
      __init__.py   你在这里：分层说明 + 入口转口
      entry.py      `expand_rule_turn_v12` = 经济层 + 军事层（四行胶水）
