...
from mp import build_econ          # 自带的 best_build（见下）要用它
...
# ---------------------------------------------------------------- 自带的 best_build
# ★ 这个函数原来住在 `mp.py`（游戏层 ROI 那一批，2026-09-11），后来被删掉了
#   （v8/v9/v10 改用 `build_econ` 自己排序）。本文件是**冻结的对照文件**，
#   不该因为引擎删了个辅助函数就 import 不进来 —— 所以把它那一版抄在这里，
#   抄的是 `8e6a734:mp.py` 里那一份、一字未改（含 `max_payback` 口径）。
