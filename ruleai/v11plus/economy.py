...
# ★★ 2026-09-26 **上面这条"装备只在征兵时一次性消耗"的认定已经过时**
#    —— 但用户口径是「**未来 v13 再处理，先留着**」，所以这里不改，只留接手须知：
#
#    同日（2026-09-26）起，军队**回血要吃装备**（`balance.HEAL_EQUIP_COST`：满足补给才回血、
#    每支每回合 1 件、民兵除外）⇒ **装备从此有常驻出口**，"存量品"的估值前提不成立了。
#    实测（100 回合 seed 42，秦楚双 v10/v11 代打，补给厂吃 2 粮）：
#
#        回血免费        ⇒ 领地 108 / 125，战报 154 / 189
#        回血吃装备(现状) ⇒ 领地  42 /  50，战报  52 /  73     ⇒ **扩张 −60%、交战 −65%**
#
#    机制：按卖价估 + ×0.85 ⇒ 装备厂 ROI 掉在采集楼之后 ⇒ **六次实验里装备厂一律 0 座**
#    （连"回血免费"那两行也是 0 —— 本层**从来**不建装备厂，一直靠市场买装备）
#    ⇒ 回血与征兵抢同一口装备（征兵 5 件/支）⇒ 库存常年 0~2 ⇒ 伤兵治不好、征兵也卡。
#
#    ⇒ **v13 该做的**：把装备跟补给一样按**流量品**估值（并复核 `STOCK_OUTPUT_FACTORIES`
#      的名单要不要拆成"每回合消耗 / 一次性消耗"两类）。**现在先留着，别动。**
...
def run(ledger, world, name: str) -> None:
    ...
    for p in free_tiles:
        ...
        if (power + gen_add) < need_pw + _dem_add:
            # ★ `spend_ok` 只吃一个参数（建筑名）——这里曾经多传了一个 `q`，
            #   而它是**惰性生成器里的短路尾巴**：只有前面 `afford` 全真时才会求值，
            #   于是这个 TypeError 在"开局没电、又有钱建电厂"之前一直没被踩到
            #   （2026-09-21 开局白送市政厅 ⇒ 第 1 回合 need_pw 就 >0 ⇒ 当场露头）。
