...
本分支（`feat/rl`）= **main 的引擎 + RL 训练层**，没有别的：

1. **引擎一字不改**（`mp.py` / `game.py` / `mp_ai.py` / `mp_run.py` 与 main 逐字节相同）。
   曾经本分支还砍掉了国家间外交，**2026-09-12 废弃了那条**：训练是单国独局，
   外交代码路径一次都不触发，砍了零收益，却要每次跟着 main 变基重做一遍外科手术。
   现在守这条的是 `tests/test_branch_behavior_parity.py::TestEngineIsMainByteForByte`。
2. **加了 RL 训练层**（`rl/`）——用 PyTorch + PPO 训一个"只认总消费"的君主。

**观测词表是冻结的**（`rl/vocab.py`）：引擎里有的项（如「外交中心」）不一定进观测，
`MAIN_ONLY` 就是那道缝。独局下造不出来、用不上的建筑不进观测/动作空间 ——
它一进去宽度就从 36/48 变 37/49，**已训好的 ckpt 全部加载不了**。
...
| 层 | 文件 | RL 用不用 |
|----|------|-----------|
| 游戏层 | `game.py`（数值表）+ `mp.py`（引擎 `World`）+ `expand_rule_v*.py`（规则 AI） | **只用这层** |
...
- **动作 = 每步枚举的合法动作清单**（candidate set）：`build / recruit / move / attack /
  ...
  **不限额**：合法动作全部进候选。曾经按类别限额（build/move 各 64）并按回合轮转起点，
  理由是"帝国越大候选越多，开销会随国力膨胀"。但那个"长期无死角"只在跨回合看时成立，
  代价是**每一步都有一批动作根本不在清单里**：

  | 后果 | 实测 |
  |------|------|
  | 后期军队动不了 | 军队数峰值 **166**，旧 `MAX_ARMIES=32` 只能指挥 19% |
  | BC 标签丢失 | 整体 12%（move 29.4% / build 33.2%） |
  | 市场商品"不存在" | 候选按商品连着排，截断后粮食/补给在某步根本没有 |

  实测候选规模（一局 500 回合、v6 驱动）：单步总数 **中位 132 / p90 890 / 峰值 1376**，
  其中 move 峰值 1001、build 峰值 427。分布是双峰的——早期几十个（便宜），
  只有后期才上千，所以放开限额的实际开销远小于按最坏情况估的值。
...
### 取消限额之后还跑得动吗（实测，Pi 5 单线程/4 线程）

候选集不再设上限后，最坏情况单步 1376 个候选，所以每一环都重新量过：

| 环节 | 成本 | 随候选数增长 |
|------|------|--------------|
| 生成观测+候选（`env._obs()`） | 1.2ms（101 候选）→ 7.7ms（676 候选） | 近似线性，基数很小 |
| 采样单步前向（batch=1） | 7.2ms（130）→ 8.9ms（1400） | **几乎不增长**——batch=1 时贵的是固定大小的卷积 |
| 更新一步前后向（batch=256） | 1.5s（300）→ 3.0s（1000） | 线性，这才是瓶颈 |

所以：**采样侧放开限额几乎不要钱**（一局 500 回合约 1 分钟），代价全在更新侧
（候选翻倍→每步翻倍）。候选分布是双峰的（中位 132、p90 890），实际开销
远低于按峰值估的值。一轮 PPO（8 局 + 更新）在服务器上约 10~15 分钟。

## 基线参考

同一局（seed 0、200 回合、单国）：规则 AI 花掉 **6261**，
随机策略约 1500——会打与乱打差 4 倍，且规则 AI 的支出仍在线性增长（资源没花完，
说明还有空间）。这是训练要超过的下限。当前最强基线是 `expand_rule_v6.py`
（五图均 ~141k 终局消费）。
