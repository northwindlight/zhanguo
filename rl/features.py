...
# ===========================================================================
# 5. 动作被拒的**原因分类**（给「上一步的反馈」当 one-hot 用，见 env.glob_channels）
# ===========================================================================
# 为什么要有：候选集放开之后（不再按钱/货预过滤，见 TOKEN_DESIGN §10.12），
# **~49% 的候选是点不动的**。没有拒绝反馈，策略就分不清「点了个无效选项」和
# 「做了个中性动作」——两种情况对模型都是"什么都没发生"，学不出甄别。
#
# 而玩家**本来就收得到**这份信息：LLM 玩家的工具返回值就是这个文案，留在它上下文里
# （`mp.py` 的 `return False, f"..."`）。所以把它编码进观测**不是"帮模型计算"**，
# 是把玩家已有的信息集补齐（契约第 2 条）。
#
# 顺序即 one-hot 下标；`其他` 必须在最后（未知文案兜底，防漂测试盯着它）。
REJECT_REASONS = ("黄金不足", "木材不足", "物资不足", "结构", "军队", "领土", "停摆", "其他")

# 关键词 → 类。**顺序敏感**：先判"停摆"（"全国电网不足…停摆"里也含"不足"）。
_REJECT_RULES = (
    ("停摆", ("电网不足", "停摆")),
    ("黄金不足", ("黄金不足",)),
    ("木材不足", ("木材不足",)),
    ("物资不足", ("储备不足", "不够：现")),
    ("领土", ("已是自己领土", "不能撤到敌国", "他国领土", "中立")),
    ("军队", ("军队", "交战", "已移动", "只能移动", "撤出", "撤退", "额度用尽")),
    ("结构", ("只能在自己的地块", "本回合已下过单", "建筑位已满", "本地无",
              "已达上限", "已用建筑位", "自建全国限", "只能在自己有兵营",
              "没有兵营", "没有军屯", "产能已用完", "未知建筑", "未知兵种",
              "不可交易", "数量需为正整数", "不存在")),
)


def reject_reason(msg: str) -> str:
    """引擎的拒绝文案 → **原因类**（不认识的一律落 `其他`，由防漂测试盯着）。

    引擎有 169 处 `return False`，但 RL 只有七个动作（build/recruit/move/attack/
    retreat/buy/sell），能触发的文案是**有限的一小撮**（见
    `tests/test_rl_env.py::TestActionFeedback` 的抽取式防漂测试）。
    """
    m = str(msg)
    for name, keys in _REJECT_RULES:
