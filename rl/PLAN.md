...
选 3M/512 就是为了"一个月内 ECS 跑得完"。有 GPU，23M/16k 那份原方案重新可议。
**建议：P4 之前不换**（P4 要答的是 yes/no，ECS 答得了；换 GPU 会跟 P4/P5 抢这一个月，
且 PPO 采样是 batch=1 逐步前向，在 GPU 上延迟绑定、未必更快；env 是单智能体，不支持并行喂 GPU）。
**P4 通了、方向确认，再换 GPU 换 P5 的迭代量**（那时要的是"20 seed × 5 配置"，不是"快 17 倍"）。
